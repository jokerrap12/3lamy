...
ksab = Token:match("(%d+)")
redis:set(ksab..":VERSION",1)
redis:set(ksab..":SUDO_ID:",SUDO_USER)
redis:set(ksab..":DataCenter:",'German')
redis:set(ksab..":UserNameBot:",BOT_User)
redis:set(ksab..":NameBot:",BOT_NAME)
redis:hset(ksab..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3ksab_INSTALL","Yes")
info = {}
...
ksab = Token:match("(%d+)")
our_id = tonumber(ksab)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(ksab..":UserNameBot:")
SUDO_ID = tonumber(redis:get(ksab..":SUDO_ID:"))
SUDO_USER = redis:hgetall(ksab..'username:'..SUDO_ID).username
version = redis:get(ksab..":VERSION")
DataCenter = redis:get(ksab..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..ksab..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(ksab..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(ksab..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(ksab..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(ksab..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(ksab..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(ksab..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(ksab..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(ksab..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(ksab..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(ksab..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(ksab..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(ksab..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(ksab..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(ksab..':WELCOME_BOT'),[[•~مـرحبآ آنآ بوت آسـمـي ]]..redis:get(ksab..':NameBot:')..[[•~آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(ksab..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(ksab..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(ksab..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(ksab..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.ksab then 
if msg.text and ScriptFile.iksab then
for k, ksab in pairs(ScriptFile.ksab) do
local SearchText = msg.text:match(ksab)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iksab(msg,{msg.text:match(ksab)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ksab.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dksab then
if ScriptFile.dksab(msg) == false then
return false
end
...
if Plug.ksab then 
if msg.text and Plug.iksab then
for k, ksab in pairs(Plug.ksab) do
local SearchText = msg.text:match(ksab)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(ksab..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(ksab..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iksab(msg,{msg.text:match(ksab)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",ksab.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source ksab")
end 

end
else
print("The File Script.lua Not Runing in The Source ksab")
...
function tdcli_update_callback(data)
	...
	redis:srem(ksab..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/mohamedadel-dev/ksab/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/mohamedadel-dev/ksab/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/mohamedadel-dev/ksab/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/mohamedadel-dev/ksab/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/mohamedadel-dev/ksab/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/mohamedadel-dev/ksab/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(ksab..":SUDO_ID:")
redis:hset(ksab..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(ksab..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(ksab..'mute_tgservice'..msg.chat_id_) then
	Del_msg(msg.chat_id_,msg.id_)
	end
	elseif msg.content_.ID == "MessageChatAddMembers" then
	if redis:get(ksab..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(ksab..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(ksab..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ksab..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(ksab..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(ksab..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(ksab..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(ksab..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(ksab..'group:add'..msg.chat_id_) then
	redis:set(ksab..'group:name'..msg.chat_id_,msg.content_.title_)
	end
	end 
	
	elseif data.ID == "UpdateNewChat" then  
	if redis:get(ksab..'group:add'..data.chat_.id_) then
	redis:set(ksab..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(ksab..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(ksab..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(ksab..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	ksab..'group:name'..k,IDS.Title,
	ksab..'num_msg_ksab'..k,5,
	ksab..'group:add'..k,true,
	ksab..'lock_link'..k,true,
	ksab..'lock_spam'..k,true,
	ksab..'lock_webpage'..k,true,
	ksab..'lock_markdown'..k,true,
	ksab..'lock_flood'..k,true,
	ksab..'lock_bots'..k,true,
	ksab..'mute_forward'..k,true,
	ksab..'mute_contact'..k,true,
	ksab..'mute_document'..k,true,
	ksab..'mute_inline'..k,true,
	ksab..'lock_username'..k,true,
	ksab..'replay'..k,true
	)
	redis:sadd(ksab..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(ksab..'username:'..ID,'username',user)
	redis:sadd(ksab..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(ksab..'username:'..ID,'username',user)
	redis:sadd(ksab..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(ksab..'username:'..ID,'username',user)
	redis:sadd(ksab..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(ksab..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(ksab..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(ksab..'username:'..data.user_.id_).username
	...
	redis:hset(ksab..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
	...
	local groups = redis:smembers(ksab..'group:ids')
	...
	local pv = redis:smembers(ksab..'users')
	local NumPvDel = 0
	for i = 1, #pv do
